...
% In the typical bias-variance decomposition of mean squared error (MSE) we treat the bias and variance of the residuals as meaningfully important features to minimize. In semantic text similarity, though, it wasn't clear to us that the \text{bias of the residuals should be centered on zero}. This reasoning is twofold. First, STS is a correlative task, so wasting training time and parameters on centering the distribution might harm generalization. Second, consider the distribution of similarity scores in STSB (SEE FIGURE !!) and notice that it is highly unnatural when compared to pairs of random sentences. 
