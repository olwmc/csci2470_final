...
For every experiment detailed here we chose to use GELU as our activation function. Our rational for this is that our base model \verb|gpt2-medium| uses GELU, so it's likely that its representation relies on the shape and quirks of that activation function. We did some experiments with other activation functions and found worse performance, suggesting that using other activation functions might waste parameters on "replicating" GELU-like transformations.
...
% One critical ethical consideration in our work is the inherent subjectivity and potential bias in how "semantic similarity" is defined and measured. The STSB dataset represents a single operationalization of semantic similarity based on human annotator judgments, but these judgments embed specific cultural, linguistic, and contextual assumptions that may not be universal.

% For example, when our model rates news headlines about different LGBT rights bills as highly similar (while STSB assigns them low similarity scores), it raises questions about what aspects of meaning we prioritize in similarity judgments. Should we focus on specific details (different locations, different bills) or broader themes (civil rights legislation)? These aren't just technical questions - they reflect deeper assumptions about what makes text "meaningfully" similar, assumptions that can vary significantly across different cultures, professional contexts, and use cases.

% This bias in similarity measurement has real implications. Models trained on STSB will inherit its particular view of semantic similarity, which could lead to systematic biases when deployed in real-world applications. A legal analysis tool might inappropriately cluster distinct legal concepts, or a content recommendation system might fail to recognize meaningful similarities across different cultural contexts. By treating semantic similarity as a single scalar value rather than acknowledging its multi-dimensional and context-dependent nature, we risk creating systems that impose one particular view of linguistic similarity on diverse user populations.

% This concern connects to broader issues of representation and power in AI systems - who gets to define what counts as "similar," and whose conceptual frameworks are being encoded into our models? While our technical approach shows promise, these questions remind us that seemingly objective metrics like semantic similarity scores often embed subjective human judgments that deserve careful ethical consideration.
...
However, there's an inherent irony in our approach: while attempting to create an interpretable measure of similarity for LLM activations, we've added another layer of complexity through our own neural networks. Though we succeeded in creating a useful similarity measure, we did so by essentially trading one black box for another. This highlights a fundamental tension in modern deep learning - sometimes making systems more capable can come at the cost of making them less interpretable.
